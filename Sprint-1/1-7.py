"""
Nicky and Dev work in a company where each member is given his income in the form of points. On Nicky's birthday, Dev decided to give some of his points as a gift. 
The number of points Dev is gifting is the total number of visible zeros visible in the string representation of the N points he received this month.
...
2(base10) = 10(base2), so the output should be 10.
...
def Cipher_Zeroes(N):
    ...
    for i in N:
        if i in "069": count += 1
        if i == "8": count += 2
    if count % 2 == 0 and count > 0: count -= 1
    elif count % 2 != 0: count += 1
    
    return format(int(count), 'b')
