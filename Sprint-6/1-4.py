...
class Group:
    ...
    def __str__(self):
        student_string = [f"{student.full_name} ({student.avg_rank}): {student.courses}" for student in self.students]
        return f"{self.title}: {student_string}"
    ...
    # @classmethod
    def serialize_to_json(self, filename):
        # data = [{'title': group.title, 'students': [student for student in group.students]} for group in list_of_groups]
        with open(filename, 'w') as json_file:
